    ...
    <div className="min-h-screen text-white pb-24" style={{ background: 'linear-gradient(180deg, #0f021e 0%, #1a0b2e 100%)' }}>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="relative">
            ...
            <div className="flex-1">
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2 mb-2">
                  <input
                    ...
                    className="text-2xl font-bold text-white bg-transparent border-b-2 border-cyan-400 focus:outline-none"
                    autoFocus
                  />
                  <button onClick={handleNameEdit} className="p-1 rounded bg-green-500/20 hover:bg-green-500/30">
                    <Check size={16} className="text-green-400" />
                  </button>
                  <button onClick={() => setIsEditingName(false)} className="p-1 rounded bg-red-500/20 hover:bg-red-500/30">
              ...
              <div className="flex items-center gap-6">
                <div className="text-center">
                  <div className="text-lg font-bold text-white">{userPosts.length}</div>
                  <div className="text-xs text-gray-500">Posts</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-white">{currentUser.friends?.length || 0}</div>
                  <div className="text-xs text-gray-500">Friends</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-white">{userTribes.length}</div>
                  <div className="text-xs text-gray-500">Tribes</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Bio */}
          {currentUser.bio && (
            <p className="mt-4 text-gray-400 text-sm leading-relaxed">{currentUser.bio}</p>
          )}
          ...
          <div className="flex items-center gap-2 mt-6">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 font-semibold hover:bg-cyan-400/20 transition-all flex items-center justify-center gap-2"
            >
              <MessageCircle size={18} />
              Message
            </button>
            <button
              className="flex-1 py-2.5 rounded-xl bg-purple-400/10 border border-purple-400/30 text-purple-400 font-semibold hover:bg-purple-400/20 transition-all flex items-center justify-center gap-2"
            >
              <Phone size={18} />
              Call
            </button>
            ...
            <div className="grid grid-cols-3 gap-2">
              {userPosts.length > 0 ? (
                userPosts.map((post) => (
                  <div
                    key={post.id}
                    className="aspect-square rounded-xl overflow-hidden cursor-pointer hover:opacity-80 transition-all border border-gray-800 hover:border-cyan-400"
                  ...
                  >
                    {post.mediaUrl ? (
                      ...
                      <div className="w-full h-full bg-gray-900/50 flex items-center justify-center p-3">
                        <p className="text-xs text-center text-gray-500 line-clamp-4">
                          {post.text}
                        </p>
                      </div>
                    )}
            ...
            <div className="space-y-3">
              {userTribes.length > 0 ? (
                userTribes.map(tribe => (
                  <div
                    key={tribe.id}
                    className="p-4 rounded-xl bg-gray-900/30 border border-gray-800 hover:border-cyan-400/30 transition-all cursor-pointer"
                    onClick={() => navigate(`/tribe/${tribe.id}`)}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-semibold text-white mb-1">{tribe.name}</h3>
                        <p className="text-sm text-gray-500">{tribe.members?.length || 0} members</p>
                      </div>
                ...
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-500">No tribes joined yet</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-3">
              {userTickets.length > 0 ? (
                userTickets.map(ticket => (
                  <div
                    key={ticket.id}
                    className="p-4 rounded-xl bg-gray-900/30 border border-gray-800"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="font-semibold text-white mb-1">{ticket.event?.name || 'Event'}</h3>
                        <p className="text-sm text-gray-500">{ticket.event?.date || 'Date TBD'}</p>
                      </div>
                      {ticket.qrCodeImage && (
                        <div className="w-16 h-16 bg-white rounded-lg p-1">
                          <img src={ticket.qrCodeImage} alt="QR" className="w-full h-full" />
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-1 rounded-full bg-cyan-400/20 text-cyan-400 text-xs font-semibold">
                        {ticket.tier}
                      </span>
